...
//to display all lines on the display
void write_display(char * line1 , char* line2,char* line3,char * line4,char* line5,char* line6){
	...
	for(int i=0;i<84;i++){
		LcdWriteData(line5[i]);
	}
	for(int i=0;i<84;i++){
		LcdWriteData(line6[i]);
	}	
...
void write_string_line(char *line , char string[]){
	...
	for(int i=0;i<16,i+){
		for(int j=0;j<5;j++){
			buff_ASCII[j]=ASCII[(int)string[i]][j] ;
		}	
		for(int k=0;k<5;k++){
			line[k+start]=buff_ASCII[k];
		}		
	}	
}
